<div class="frames"<?php if(!isset($_GET['step']) || $_GET['step'] !== 'frame'){ ?> style="display:none"<?php } ?>>
	...
	<table id="receipt-summary" class="table table-bordered summary">
		<tbody>
			...
			<tr>
				<th>签收须知</th>
				<td colspan="3" class="text-left">1. 确认所收器架及其数量 <br> 2. 在右侧确认栏中点击以勾选 <br> 3. 所有器架全部确认勾选后，点击底部签收完成</td>
			</tr>
		</tbody>
	</table>
</div>
...
<div class="frames"<?php if(!isset($_GET['step']) || $_GET['step'] !== 'picture'){ ?> style="display:none"<?php } ?>>
	...
	<table id="receipt-summary" class="table table-bordered summary">
		<tbody>
			...
			<tr>
				<th>签收须知</th>
				<td colspan="3" class="text-left">1. 确认所收画面及其数量 <br> 2. 在右侧确认栏中点击以勾选 <br> 3. 所有画面全部确认勾选后，点击底部签收完成</td>
			</tr>
		</tbody>
	</table>
</div>
